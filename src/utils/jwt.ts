...
export const generateAccessToken = (userId: mongoose.Types.ObjectId | string, name: string, email: string, organizacion: mongoose.Types.ObjectId | string, role: 'user' | 'admin') => {
    const payload: IJwtPayload = { id: String(userId), name, email, organizacion: String(organizacion), role };
    return jwt.sign(
        payload,
